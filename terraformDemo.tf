...
resource "aws_vpc" "terraformDemo_vpc" {
...
resource "aws_subnet" "terraformDemo_appSubnet" {
    tags = {
        Name = "APP_Subnet"
    }
    vpc_id = aws_vpc.terraformDemo_vpc.id
    cidr_block = "10.0.1.0/24"
    map_public_ip_on_launch = true
    depends_on= [aws_vpc.terraformDemo_vpc]
    
}

#Define routing table
resource "aws_route_table" "terraformDemo_route-table" {
    tags = {
...
resource "aws_route_table_association" "App_Route_Association" {
  subnet_id      = aws_subnet.terraformDemo_appSubnet.id 
  route_table_id = aws_route_table.terraformDemo_route-table.id
}


#Create internet gateway for servers to be connected to internet
resource "aws_internet_gateway" "terraformDemo_IG" {
    tags = {
        Name = "TERRAFORM_IGW"  
    }
     vpc_id = aws_vpc.terraformDemo_vpc.id
...
resource "aws_route" "default_route" {
  ...
  gateway_id = aws_internet_gateway.terraformDemo_IG.id
}
...
resource "aws_ebs_volume" "terraformDemo_ebs" {
...
resource "aws_volume_attachment" "attach_ebs" {
  depends_on = [aws_ebs_volume.terraformDemo_ebs]
  device_name = "/dev/sdh"
  volume_id   = aws_ebs_volume.terraformDemo_ebs.id
...
resource "null_resource" "nullmount" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo git clone https://github.com/khaziwallis/testterraform.git  /var/www/html"
    ]
  }
}
...
resource "aws_s3_bucket" "terraform-racker-new-bucket" {
  bucket = "terraform-racker-new-bucket"
...
#   region = "us-east-2"
  ...
  tags = {
    Name = "terraform-racker-new-bucket"
    Environment = "Prod"
  }

 provisioner "local-exec" {
    command = "git clone https://github.com/khaziwallis/testterraform.git web-server-image"
 }
...
resource "aws_s3_bucket_public_access_block" "public_storage" {
 depends_on = [aws_s3_bucket.terraform-racker-new-bucket]
 bucket = "terraform-racker-new-bucket"
...
resource "aws_s3_bucket_object" "Object1" {
  ...
  acl    = "public-read-write"
  key = "sampleImage.png"
  source = "web-server-image/sampleImage.png"
}
...
resource "aws_cloudfront_distribution" "tera-cloufront1" {
    ...
    origin {
        domain_name = aws_s3_bucket.terraform-racker-new-bucket.bucket_regional_domain_name
